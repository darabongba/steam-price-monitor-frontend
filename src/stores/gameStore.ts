...
import { Game, GameDetails, GameSearchResult, LocalGame } from '@/types/game';
...
interface GameState {
  ...
  updateGame: (id: string, updates: Partial<LocalGame>) => Promise<void>;
    ...
    persist(
      (set) => ({
        ...
        searchGames: async (query: string) => {
          ...
          try {
            const results = await staticDataService.searchGames(query);
        ...
        //
        //   try {
        //     const gameDetails = await staticDataService.getGameDetails();
        //     if (gameDetails) {
        ...
        loadPopularGames: async () => {
          ...
          try {
            const popular = await staticDataService.getPopularGameDetails();
        ...
        addGame: async (game: Game) => {
          try {
            ...
            const newGame = { ...gameWithId, id: gameId };
        ...
        // 更新游戏信息
        updateGame: async (id: string, updates: Partial<LocalGame>) => {
          try {
        ...
        loadSavedGames: async () => {
          ...
          try {
            const savedGames = await storageService.getAllGames() as LocalGame[];
